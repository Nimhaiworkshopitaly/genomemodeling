...
from simulation_block_translocation_composite import run_simulation  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--translocation-size-mode",
        choices=("single_gene", "powerlaw", "geometric", "lognormal"),
        default="powerlaw",
    )
    parser.add_argument("--translocation-geometric-mean", type=float, default=2.0)
    parser.add_argument("--translocation-lognormal-median", type=float, default=3.0)
    parser.add_argument("--translocation-lognormal-sigma", type=float, default=1.0)
    parser.add_argument("--translocation-min-size", type=int, default=1)
    ...
    parser.add_argument("--gain-geometric-mean", type=float, default=1.0)
    parser.add_argument("--gain-lognormal-median", type=float, default=3.0)
    parser.add_argument("--gain-lognormal-sigma", type=float, default=1.0)
    parser.add_argument("--gain-min-size", type=int, default=1)
    ...
    if args.translocation_geometric_mean < args.translocation_min_size:
        parser.error(
            "--translocation-geometric-mean must be >= --translocation-min-size"
        )
    if (args.translocation_lognormal_median <= 0
            or args.translocation_lognormal_sigma <= 0):
        parser.error("translocation log-normal median and sigma must be positive")
    if args.translocation_min_size < 1:
        parser.error("--translocation-min-size must be positive")
    if args.gain_geometric_mean < args.gain_min_size:
        parser.error("--gain-geometric-mean must be >= --gain-min-size")
    if args.gain_lognormal_median <= 0 or args.gain_lognormal_sigma <= 0:
        parser.error("gain log-normal median and sigma must be positive")
    if args.gain_min_size < 1:
        parser.error("--gain-min-size must be positive")
...
def worker_simulate(payload):
    (
        ...
        translocation_size_mode, translocation_geometric_mean,
        translocation_lognormal_median, translocation_lognormal_sigma,
        translocation_min_size,
        gain_size_mode, gain_geometric_mean, gain_lognormal_median,
        gain_lognormal_sigma, gain_min_size,
    ...
    for _ in range(n_runs):
        ...
        simulated_pairs = run_simulation(
            ...
            translocation_size_mode=translocation_size_mode,
            translocation_geometric_mean=translocation_geometric_mean,
            translocation_lognormal_median=translocation_lognormal_median,
            translocation_lognormal_sigma=translocation_lognormal_sigma,
            translocation_min_size=translocation_min_size,
            gain_size_mode=gain_size_mode,
            gain_geometric_mean=gain_geometric_mean,
            gain_lognormal_median=gain_lognormal_median,
            gain_lognormal_sigma=gain_lognormal_sigma,
            gain_min_size=gain_min_size,
        )
...
def main() -> None:
    ...
    for count in split_counts(args.n_runs, args.workers):
        ...
        payloads.append((
            ...
            args.gain_loss_exp,
            args.translocation_size_mode, args.translocation_geometric_mean,
            args.translocation_lognormal_median,
            args.translocation_lognormal_sigma, args.translocation_min_size,
            args.gain_size_mode, args.gain_geometric_mean,
            args.gain_lognormal_median, args.gain_lognormal_sigma,
            args.gain_min_size,
    ...
    row = {
        ...
        "translocation_size_mode": args.translocation_size_mode,
        "translocation_geometric_mean": args.translocation_geometric_mean,
        "translocation_lognormal_median": args.translocation_lognormal_median,
        "translocation_lognormal_sigma": args.translocation_lognormal_sigma,
        "translocation_min_size": args.translocation_min_size,
        ...
        "gain_size_mode": args.gain_size_mode,
        "gain_geometric_mean": args.gain_geometric_mean,
        "gain_lognormal_median": args.gain_lognormal_median,
        "gain_lognormal_sigma": args.gain_lognormal_sigma,
        "gain_min_size": args.gain_min_size,
