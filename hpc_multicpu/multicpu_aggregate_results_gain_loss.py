#!/usr/bin/env python3
"""Combine gain/loss/rt per-job CSV files and rank parameter settings."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate gain/loss grid CSVs.")
    parser.add_argument("inputs", nargs="+", help="CSV files or glob patterns")
    parser.add_argument(
        "--out-csv",
        default="hpc_multicpu/combined_gain_loss_ranked.csv",
    )
    parser.add_argument(
        "--sort-by",
        default="avg_w1",
        choices=["avg_w1", "sum_w1", "composite_score"],
    )
...
def main() -> None:
    ...
    if args.sort_by not in combined.columns:
        raise SystemExit(f"Column '{args.sort_by}' not found.")
