...
def evolve_genome_branch(
    ...
    translocation_size_mode="powerlaw", translocation_geometric_mean=2.0,
    translocation_lognormal_median=3.0, translocation_lognormal_sigma=1.0,
    translocation_min_size=1,
    gain_size_mode="powerlaw", gain_geometric_mean=1.0,
    gain_lognormal_median=3.0, gain_lognormal_sigma=1.0,
    gain_min_size=1,
):
    genome = genome.copy()
    ...
    for event in events:
        ...
        if event == "I" and inversion_size_mode == "uniform_breakpoints":
        ...
        elif event == "T" and translocation_size_mode == "single_gene":
            k = 1

        elif event == "T" and translocation_size_mode == "geometric":
            k = sample_geometric_inversion(
                Lc, translocation_geometric_mean, translocation_min_size
            )
            if k is None:
                continue

        elif event == "T" and translocation_size_mode == "lognormal":
            k = sample_lognormal_inversion(
                Lc, translocation_lognormal_median,
                translocation_lognormal_sigma, translocation_min_size,
            )
            if k is None:
                continue

        elif event == "G" and gain_size_mode == "single_gene":
            k = 1

        elif event == "G" and gain_size_mode == "geometric":
            k = sample_geometric_inversion(
                Lc, gain_geometric_mean, gain_min_size
            )
            if k is None:
                continue

        elif event == "G" and gain_size_mode == "lognormal":
            k = sample_lognormal_inversion(
                Lc, gain_lognormal_median, gain_lognormal_sigma, gain_min_size
            )
            if k is None:
                continue

        elif event == "G":
            k = sample_truncated_powerlaw(L0_ancestral, gain_exp, gain_min_size)
        elif event == "L":
            ...
            k = sample_truncated_powerlaw(
                L0_ancestral, trans_exp, translocation_min_size
            )
