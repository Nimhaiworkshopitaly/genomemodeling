#!/usr/bin/env python3
"""Joint translocation/inversion grid with strict empirical core COG protection."""
...
RESULTS = Path("hpc_multicpu/results_joint_translocation_inversion_uniform_empirical_core")
FIGURES = Path("hpc_multicpu/figures_joint_translocation_inversion_uniform_empirical_core")
SWARM = Path("hpc_multicpu/jobs_joint_translocation_inversion_uniform_empirical_core.swarm")
...
def load_results():
    ...
    numeric_checks = {
        "rf": 0.1, "core_fraction": 0.0, "core_protection": 0.9,
        "core_prevalence": 1.0,
    ...
    for column, expected_value in {
        ...
        if not data[column].eq(expected_value).all():
            raise ValueError(f"Unexpected values in {column}.")
    if (data["empirical_core_count"] <= 0).any():
        raise ValueError("No empirical core COGs were identified in some results.")
    if (data["root_core_count"] <= 0).any():
        raise ValueError("No empirical core COGs overlap the selected root in some results.")
    for column in ["empirical_core_count", "root_core_count", "root_core_fraction"]:
        if data[column].nunique() != 1:
            raise ValueError(f"Inconsistent {column} across matched settings.")
    expected_tree_suffix = "ATGC0070/yuri_gl26/ATGC0070.gl.tre"
    normalized_trees = data["tree_path"].str.replace("\\\\", "/", regex=False)
    if not normalized_trees.str.endswith(expected_tree_suffix).all():
        raise ValueError("Some results do not use Yuri's gain+loss-clock tree.")
...
def analyze() -> None:
    ...
    for metric, label in metrics:
        ...
        ax.set_title(
            f"{label}: mean across five matched seeds\n"
            "Lower is better; gain/loss rates fixed at 0.1; empirical core COG protection"
        )
