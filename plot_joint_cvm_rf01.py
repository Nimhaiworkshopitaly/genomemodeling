#!/usr/bin/env python3
"""Plot two joint-model Cramer-von Mises heatmaps for rf=0.1."""
...
METRIC = "avg_cramer_von_mises_distance"
...
def main():
    ...
    parser.add_argument(
        ...
        default=Path(
            "hpc_multicpu/"
            "figures_joint_size_cvm_rf_0p1_observed_medoid_empirical_core/"
            "rf_0p1_cvm_joint_model_two_panel"
        ),
    )
    args = parser.parse_args()
    data = pd.read_csv(args.input)
    panels = {gain: matrix_and_optimum(data, gain) for gain in GAIN_ORDER}
    values = np.concatenate([panels[gain][0].to_numpy().ravel() for gain in GAIN_ORDER])
    ...
    colorbar.set_label(
        "Best mean Cramer-von Mises distance across rate combinations; lower is better"
    )
    fig.suptitle(
        "Joint translocation- and inversion-size Cramer-von Mises landscapes\n"
